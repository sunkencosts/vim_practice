...
2. For each motion covered, ask the user how it felt one question at a time. Ask about all of them in a single message rather than one at a time — list each motion with its current comfort level from `progress.md` and ask them to reply with the new level for any that changed. Use this scale:
